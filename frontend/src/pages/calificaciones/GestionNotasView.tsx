...
import type { Calificacion, NotaInput } from './services/calificacionesService';
...
const GestionNotasView: FC = (): ReactElement => {
  ...
  const [notasExistentes, setNotasExistentes] = useState<Calificacion[]>([]);
  ...
  const construirGrid = useCallback((estudiantesLista: EstudianteInfo[], notas: Calificacion[], cols: ColumnaEval[]): GridData => {
  ...
  const handleSeleccionarMateria = useCallback(async (mat: FlatMateria) => {
    ...
    try {
      ...
      const [estudiantesData, notasData] = await Promise.all([
        courseService.obtenerEstudiantesPorCurso(mat.curso_nombre),
        calificacionesService.obtenerNotasCurso(mat.curso_nombre, profesorRut),
      ]);
      ...
      const notasFiltradas = (Array.isArray(notasData) ? notasData : []).filter(
        (n: Calificacion) => n.asignatura === mat.asignatura_nombre,
      );
  ...
  const handleGuardarTodo = async () => {
    ...
    const aCrear: NotaInput[] = [];
    const aActualizar: { id: number; datos: Partial<Calificacion> }[] = [];
    ...
    try {
      if (aCrear.length > 0) {
        await calificacionesService.crearNotasBatch(aCrear);
      }
      for (const item of aActualizar) {
        await calificacionesService.actualizarNota(item.id, item.datos);
      }
