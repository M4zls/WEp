...
import type { Asistencia } from '../../asistencia/types';
import { ESTADOS_CLASE } from '../../clases/types';
import { DIAS_SEMANA } from '../../horario/types';
...
const SubjectDetail: FC = (): ReactElement | null => {
  ...
  useEffect(() => {
    ...
    if (effectiveRole === 'profesor' && state?.courseName) {
      courseService.obtenerEstudiantesPorCurso(state.courseName)
  ...
  return (
    <DashboardLayout userData={userData} role={role} onLogout={handleLogout} defaultSection={role === 'profesor' ? 'cursos' : 'clases'}>
      <div className="mb-6">
        <button
          onClick={() => navigate('/dashboard', { state: { section: role === 'profesor' ? 'cursos' : 'clases' } })}
                  ...
                  {clases.sort((a, b) => b.fecha.localeCompare(a.fecha)).map(cl => (
                    <button
                      ...
                      onClick={async () => {
                        ...
                        try {
                          const data = await asistenciaService.listarPorClase(cl.id);
                          setAsistencias(data);
                        } catch {
                          setAsistencias([]);
                        } finally {
                      ...
                      <button
                        onClick={async () => {
                          ...
                          try {
                            ...
                            await asistenciaService.marcar({
                              claseId: asistenciaClaseId,
                              cursoAsignaturaId: id,
                              registros,
                            });
                            const updated = await asistenciaService.listarPorClase(asistenciaClaseId);
