...
const ClaseFormModal: FC<ClaseFormModalProps> = ({ isOpen, onClose, onSave, editingClase }): ReactElement | null => {
  ...
  const [estado, setEstado] = useState(CLASS_STATUSES.PENDING);
  ...
  useEffect(() => {
    if (editingClase) {
      ...
      setEstado(editingClase.estado || CLASS_STATUSES.PENDING);
    } else {
      setTitulo('');
      setDescripcion('');
      setFecha('');
      setHoraInicio('');
      setHoraTermino('');
      setEstado(CLASS_STATUSES.PENDING);
    }
    setError('');
  }, [editingClase, isOpen]);
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          {editingClase && (
            <div>
              ...
              >
                <option value={CLASS_STATUSES.PENDING}>Pendiente</option>
                <option value={CLASS_STATUSES.COMPLETED}>Realizada</option>
                <option value={CLASS_STATUSES.CANCELLED}>Cancelada</option>
              </select>
            </div>
          )}
