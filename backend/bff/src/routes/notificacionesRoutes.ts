...
app.post('/aviso-nota', async (c) => {
  const body = await c.req.json();
  const response = await fetch(`${NOTIFICACIONES_SERVICE}/notificaciones/aviso-nota`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(body),
  });
...
app.get('/usuario/:usuarioId', async (c) => {
  ...
  const response = await fetch(`${NOTIFICACIONES_SERVICE}/notificaciones/usuario/${usuarioId}`);
  const data = await response.json();
  return c.json(data, response.status as any);
});
...
app.get('/usuario/:usuarioId/no-leidas', async (c) => {
  const usuarioId = c.req.param('usuarioId');
  const response = await fetch(`${NOTIFICACIONES_SERVICE}/notificaciones/usuario/${usuarioId}/no-leidas`);
  const data = await response.json();
  return c.json(data, response.status as any);
});
...
app.put('/:id/leer', async (c) => {
  ...
  const response = await fetch(`${NOTIFICACIONES_SERVICE}/notificaciones/${id}/leer`, {
    method: 'PUT',
  });
  const data = await response.json();
  return c.json(data, response.status as any);
