...
app.get('/clase/:claseId', async (c) => {
  try {
    ...
    const response = await fetch(`${ASISTENCIA_SERVICE}/asistencia/clase/${claseId}`);
...
app.get('/estudiante/:rut', async (c) => {
  try {
    ...
    const response = await fetch(`${ASISTENCIA_SERVICE}/asistencia/estudiante/${rut}`);
    const data = await response.json();
    return c.json(data, response.status as any);
  } catch (error) {
...
app.get('/curso-asignatura/:id', async (c) => {
  try {
    ...
    const response = await fetch(`${ASISTENCIA_SERVICE}/asistencia/curso-asignatura/${id}`);
    const data = await response.json();
    return c.json(data, response.status as any);
  } catch (error) {
...
app.post('/marcar', async (c) => {
  try {
    ...
    const response = await fetch(`${ASISTENCIA_SERVICE}/asistencia/marcar`, {
      method: 'POST',
...
app.put('/:id', async (c) => {
  try {
    const id = c.req.param('id');
    const body = await c.req.json();
    const response = await fetch(`${ASISTENCIA_SERVICE}/asistencia/${id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(body),
    });
