...
export const crearHorarioSchema = z.object({
  ...
  diaSemana: z.number({ error: 'El día de semana es obligatorio' }).int()
    .refine(d => diasValidos.includes(d as any), 'Debe ser 1=Lun, 2=Mar, 3=Mié, 4=Jue, 5=Vie'),
  horaInicio: z.string({ error: 'La hora de inicio es obligatoria' }).regex(/^\d{2}:\d{2}$/, 'Formato HH:MM'),
  horaTermino: z.string({ error: 'La hora de término es obligatoria' }).regex(/^\d{2}:\d{2}$/, 'Formato HH:MM'),
});

export const actualizarHorarioSchema = z.object({
  cursoAsignaturaId: z.number().int().positive().optional(),
