...
async function seed() {
  ...
  const estudiantes = await sql.unsafe('SELECT rut, nombre, apellido FROM "estudiantes"."estudiantes" LIMIT 1');
  ...
  const profesores = await sql.unsafe('SELECT rut, nombre, apellido FROM "profesores"."profesores" LIMIT 1');
