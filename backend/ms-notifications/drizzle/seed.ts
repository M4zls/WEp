...
async function seed() {
  ...
  const existing = await sql`SELECT COUNT(*) as count FROM "notificaciones"."notificaciones"`;
  ...
  await sql`
    INSERT INTO "notificaciones"."notificaciones" (usuario_id, tipo, titulo, mensaje, leida, fecha_creacion) VALUES
